...
static const char *TAG = "esp32_sensor_real_init";
...
static int omv_read_reg(omv_sensor_t *sensor,uint16_t reg_addr)
{
    sensor_t *esp_sensor = esp_camera_sensor_get();
    ESP_SENSOR_OP(get_reg(esp_sensor, reg_addr, ~0U));
}

static int omv_write_reg(omv_sensor_t *sensor,uint16_t reg_addr, uint16_t reg_data)
{
    sensor_t *esp_sensor = esp_camera_sensor_get();
    ESP_SENSOR_OP(set_reg(esp_sensor, reg_addr, ~0U, reg_data));
}
...
static int omv_set_auto_gain(omv_sensor_t *sensor, int enable, float gain_db, float gain_db_ceiling)
{
  sensor_t *esp_sensor = esp_camera_sensor_get();
  ESP_SENSOR_OP(set_gain_ctrl(esp_sensor, enable))
}
